...
5. Bonus : 위 점수가 63점을 넘을시 35점 추가.
   Yacht Bonus : Yacht가 또 나올경우 100점 추가.
'''
        ...
        self.yacht_l = QLineEdit(self)
        self.yachtBonus_l = QLineEdit(self)
        ...
        self.lineEditList = [
            ...
            self.l_s_l, self.yacht_l, self.yachtBonus_l, self.score_l
        ]
        
        self.d0_img = QPixmap('Python/yacht_Game/D0.png')
        self.d1_img = QPixmap('Python/yacht_Game/D1.png')
        self.d2_img = QPixmap('Python/yacht_Game/D2.png')
        self.d3_img = QPixmap('Python/yacht_Game/D3.png')
        self.d4_img = QPixmap('Python/yacht_Game/D4.png')
        self.d5_img = QPixmap('Python/yacht_Game/D5.png')
        self.d6_img = QPixmap('Python/yacht_Game/D6.png')
        ...
        # 야찌 보너스 점수판
        self.yachtBonus = QLabel('Yacht Bonus :')
        self.lineEditList[r].setReadOnly(True)
        barrier = QLabel('｜')
        score_grid.addWidget(self.yachtBonus, r, 0)
        score_grid.addWidget(self.lineEditList[r], r, 1)
        score_grid.addWidget(barrier, r, 2)
        r += 1
        ...
        for i in range(len(s)):
                self.saver.append(f"{i+1}p : {s[i][1]}s\t {s[i][0]} recorded")
        ...
        self.roll_c.setReadOnly(True)
        reroll_c_m = QLabel('count :')
        self.reroll_c = QLineEdit('2')
        self.reroll_c.setReadOnly(True)
        ...
        if key == 'Roll !':
            ...
            if self.lineEditMap[key].text() == '' and self.d_list[0] != '0':
                ...
                if len(set(self.d_list)) == 1:
                    ...
                    if self.cyacht > 1:
                        self.ybonus += 100
                        self.yachtBonus_l.setText(str(self.ybonus))
                ...
                sc = calcFunctions.score(self.aces_l.text(), self.twos_l.text(), self.threes_l.text(),  \
                    ...
                    self.l_s_l.text(), self.yacht_l.text(), self.Bonus_l.text(), self.yachtBonus_l.text())
...
if __name__ == '__main__':
    ...
    game.resize(500, 700)
    game.move(700,150)
    game.setWindowTitle('Yacht_Game')
    game.show()
    sys.exit(app.exec_())